...
export default function AgendaEntregas({ rol }) {
  ...
  const abrirWhatsApp = (telefono, nombre, producto) => {
    ...
    const msg = encodeURIComponent(`Hola ${primerNombre}! ­ƒÑª Estuvimos armando tu pedido de *${producto || ''}* de Huerta Urbana. Nos pondremos en contacto pronto por la entrega.`);
  ...
  const imprimir = (alcance) => {
    ...
    const renderHojasPedido = (p) => {
      const crearHoja = (tipo) => `
        <div class="hoja">
          <div class="header">
            <div class="header-left">­ƒî┐ HUERTA URBANA<div class="header-web">huertaurbana.com.ar | Tel: 11 6177-1376</div></div>
          ...
          <div class="client-data">
            ...
            <div class="client-info"><strong>DIRECCI├ôN:</strong> ${p.direccion || ""}, ${p.localidad || ""}</div>
            <div class="client-info"><strong>TEL├ëFONO:</strong> ${p.telefono || ""}</div>
            <div class="client-info"><strong>ENTREGA:</strong> ${p.dia_entrega || ""} (${p.horario_entrega || ""})</div>
            <div class="client-info"><strong>PAGO:</strong> APROBADO Ô£à</div>
            ${p.observaciones ? `<div class="client-info"><strong>OBS:</strong> ${p.observaciones}</div>` : ""}
          </div>
          <div class="products-section">
            <div class="products-title">Productos:</div>
            <table>
              <thead><tr><th style="width:50px;text-align:center">Cant</th><th>Descripci├│n</th><th style="width:100px;text-align:right">Subtotal</th></tr></thead>
          ...
          <div class="footer">
            <div class="signature-row"><div>Recib├¡ conforme: <span class="linea-puntos" style="min-width: 200px"></span></div></div>
            <div class="signature-row"><div>Firma: <span class="linea-puntos" style="min-width: 180px"></span></div><div>Aclaraci├│n: <span class="linea-puntos" style="min-width: 220px"></span></div></div>
            <div class="thanks-footer">┬íGracias por tu compra! ­ƒî┐ <strong>huertaurbana.com.ar</strong></div>
          </div>
        </div>
      `;
    ...
    <div className="p-1 sm:p-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">{rol === 'repartidor' ? 'Vista de Reparto' : 'Gesti├│n Operativa e Impresi├│n'}</p>
        </div>
      ...
      {pedidosDelTurno.length === 0 ? (
        ...
        <div className="space-y-3">
          {pedidosDelTurno.map(p => {
            ...
            const configEstado = {
              pendiente:    { icon: '­ƒƒí', label: 'PREPARADO', color: 'text-amber-400',  bg: 'bg-amber-500/5',  border: 'border-amber-500/10' },
              preparado:    { icon: '­ƒƒí', label: 'PREPARADO', color: 'text-amber-400',  bg: 'bg-amber-500/5',  border: 'border-amber-500/10' },
              listo:        { icon: '­ƒƒí', label: 'PREPARADO', color: 'text-amber-400',  bg: 'bg-amber-500/5',  border: 'border-amber-500/10' },
              entregado:    { icon: 'Ô£à', label: 'ENTREGADO', color: 'text-green-500',  bg: 'bg-green-500/10', border: 'border-green-500/30' },
              no_entregado: { icon: 'ÔØî', label: 'NO ENTREGADO', color: 'text-red-500', bg: 'bg-red-500/10',   border: 'border-red-500/30' },
            };
                ...
                <div className="px-4 py-4 flex items-center justify-between cursor-pointer" onClick={() => toggleAcordeon(p.numero_pedido)}>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className={`text-[11px] ${riderBg ? 'text-black/60' : 'text-gray-500'}`}>{p.direccion} ÔÇó {p.localidad}</p>
                    </div>
                  </div>
