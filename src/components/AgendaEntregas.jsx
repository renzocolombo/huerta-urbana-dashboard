        ...
        <div className="space-y-3">
          {pedidosDelTurno.map(p => {
            ...
            if (rol === 'repartidor') {
              ...
              else if (estadoActual === 'no_entregado') riderBg = 'bg-[#fee2e2] !text-black border-red-300';
            }
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <p className={`font-bold transition-all ${riderBg ? 'text-black text-lg' : 'text-white text-sm'}`}>{p.nombre}</p>
                        <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-lg border ${riderBg ? 'bg-white/40 border-black/20 text-black' : 'bg-black/40 border-white/10 text-white'}`}>
                          {conf.icon} {conf.label}
                        </span>
                      </div>
                      <p className={`text-[11px] font-medium ${riderBg ? 'text-black/60' : 'text-gray-500'}`}>{p.direccion} • {p.localidad}</p>
                    </div>
                  </div>
                ...
                {isOpen && (
                  <div className="px-4 pb-5 pt-2 border-t border-black/5 space-y-4">
                    ...
                    <div className="flex gap-2 items-stretch">
                      <button onClick={() => abrirWhatsApp(p.telefono, p.nombre, p.producto)} className="flex-1 flex items-center justify-center gap-2 bg-[#25D366] text-white font-bold py-3 rounded-xl shadow-lg transition-all active:scale-95">
                        <MessageCircle size={18} /> WhatsApp
                      </button>
                      {rol !== 'repartidor' && (
                        <div className="bg-black/40 border border-white/10 p-3 rounded-xl flex-1 text-right flex flex-col justify-center">
